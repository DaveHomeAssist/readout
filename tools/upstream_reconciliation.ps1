...
$ahead = [int]$parts[0]
$behind = [int]$parts[1]
Add-Result -Check "Graph" -Result ($(if ($behind -eq 0) { "PASS" } else { "REVIEW" })) -Detail "ahead=$ahead; behind=$behind"

$status = Invoke-GitText -GitArgs @("status", "--short")
$dirtyCount = @($status | Where-Object { $_ }).Count
Add-Result -Check "Worktree" -Result ($(if ($dirtyCount -eq 0) { "PASS" } else { "REVIEW" })) -Detail "$dirtyCount changed/untracked path(s)"
...
$runtimeDetail = if ($runtimeDelta.Count -eq 0) { "none" } else { (($runtimeDelta -join ", ") -replace "\|", "/") }
Add-Result -Check "Runtime-sensitive upstream paths" -Result ($(if ($runtimeDelta.Count -eq 0) { "PASS" } else { "REVIEW" })) -Detail $runtimeDetail
...
$reviewRows = @($results | Where-Object { $_.Result -ne "PASS" })
