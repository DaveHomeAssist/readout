...
foreach ($script in @("tools\secret_scan.ps1", "tools\architect_signoff_check.ps1", "tools\packaging_validation_check.ps1", "tools\manual_smoke_check.ps1", "tools\roadmap_audit.ps1", "tools\upstream_reconciliation.ps1", "tools\cors_origin_matrix.ps1", "tools\server_smoke.ps1", "tools\control_workflow_smoke.ps1", "tools\extension_static_smoke.ps1", "tools\tk_desktop_static_smoke.ps1", "tools\tk_desktop_runtime_smoke.ps1", "tools\windows_packaging_prereqs.ps1", "tools\windows_package_smoke.ps1", "tools\release_preflight.ps1", "build_windows.ps1")) {
    try {
...
try {
    $exitCode = Invoke-PreflightCommand { & .\tools\tk_desktop_static_smoke.ps1 -Quiet }
    Add-Check -Check "Tk desktop static smoke" -Result ($(if ($exitCode -eq 0) { "PASS" } else { "FAIL" })) -Detail "tk_desktop_static_smoke.ps1 exit=$exitCode"
} catch {
    Add-Check -Check "Tk desktop static smoke" -Result "FAIL" -Detail $_.Exception.Message
}
