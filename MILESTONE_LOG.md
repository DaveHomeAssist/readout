...
## 2026-06-24 00:30 -04:00 - macOS Package Tray Evidence Cleared
- **Workstream:** P3-A1 macOS packaging and final roadmap gate evidence.
- **What changed:** Updated `PACKAGING_VALIDATION.md`, `ROADMAP_STATUS.md`, `ARCHITECT_SIGNOFF.md`, and `NEXT_EXECUTOR_PROMPT.md` to record the successful hosted macOS tray UI package smoke. Patched `tests/test_server_api.py::test_status_reports_load_error` so the test stubs dependency checks and remains deterministic on the lightweight CI test environment without Kokoro/eSpeak installed.
- **Evidence:** GitHub Actions package-smoke run `28074903385` passed on commit `10d7550`. macOS job `83117008362` built `dist/ReadOut.app`, ran `./tools/mac_package_smoke.sh --app dist/ReadOut.app --include-audio --include-tray-ui --evidence-dir macos-tray-evidence`, passed `Menu-bar/tray icon visible`, passed tray `Open Control Panel` to `/control`, passed preview/stop/speak/stop lifecycle, blocked-origin, and clean quit, then uploaded artifact `readout-macos-package-smoke` id `7840118532` with digest `sha256:5868986c68411cd2ee7370a36835ecbcf2017c5335a72de9e9bf79124bcfd369`. Windows job `83117008329` passed and uploaded artifact `readout-windows-package-smoke` id `7840090908` with digest `sha256:d8cadd4e45240e21eb31806b19ef499b99d5ad4e066bdaa08b155cacbacd7894`.
- **Next:** Run local release gates after the documentation/test update, commit and push the evidence cleanup, then confirm the latest GitHub tests workflow is green for the new head.
- **Blockers:** No packaging worksheet row is intentionally open after this evidence update. Final status still depends on current local release checks and latest GitHub tests passing after this commit.
