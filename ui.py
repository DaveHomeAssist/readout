...
ENGINES = {
    ...
    "browser":    {"label": "Browser",    "badge": "FREE",   "badge_fg": C_ACCENT, "badge_bg": "#1a2a0a"},
}
...
VOICES_BY_ENGINE = {
    ...
    "browser":    ["Default System Voice", "Google US English", "Google UK English"],
}
...
class ReadOutApp(tk.Tk):
    ...
    def _build_guide(self, parent):
        ...
        sections = [
            ...
            ("SAVING AUDIO", [
                "• Click Save MP3 in the desktop UI",
    ...
    def _select_engine(self, key: str):
        ...
        menu.delete(0, "end")
        for v in voices:
            menu.add_command(label=v, command=lambda val=v: self._voice.set(val))
        if voices:
    ...
    def _build_playback_bar(self, parent):
        ...
        self._auto_btn.pack(side="left", fill="x", expand=True, padx=(0, 6), ipady=7)
    ...
    def _play(self):
        ...
        voice_label = self._voice.get()
        voice_id    = voice_label.split(" ")[0]  # strip description
    ...
    def _save_audio(self):
        text     = self._get_text()
        voice_id = self._voice.get().split(" ")[0]
        speed    = round(self._speed.get(), 1)
    ...
    def _on_saved(self, path: str):
        if path:
            ...
            self._save_btn.configure(text="⬇  Save MP3", fg=C_DIM)
        self.after(2000, lambda: self._save_btn.configure(
            text="⬇  Save MP3", fg=C_DIM))

    def _toggle_auto(self):
        self._auto_var.set(not self._auto_var.get())
        if self._auto_var.get():
            self._auto_btn.configure(fg=C_ACCENT, bg="#1a2a0a")
        else:
            self._auto_btn.configure(fg=C_DIM, bg=C_SURFACE2)
    ...
    def _render_queue(self):
        ...
        for i, item in enumerate(self._queue):
            ...
            # Download button
            tk.Button(row, text="↓", bg=row["bg"], fg=C_MUTED,
                      relief="flat", bd=0, font=("Helvetica", 10),
                      cursor="hand2", pady=4).pack(side="right", padx=(0, 8))
