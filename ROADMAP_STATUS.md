...
| ID | Status | Current Evidence | Remaining Proof |
|---|---|---|---|
| P3-A1 | Complete hosted macOS validation | `build_mac.sh` checks supported Python and `espeak-ng`; build-script tests cover preflight expectations; packaged entrypoint tests prove tray/control-panel routing; GitHub Actions package-smoke run `28074903385` built `dist/ReadOut.app`, passed `tools/mac_package_smoke.sh --app dist/ReadOut.app --include-audio --include-tray-ui --evidence-dir macos-tray-evidence`, verified `/control`, preview/stop/speak/stop audio lifecycle, visible menu-bar/tray, tray `Open Control Panel` callback to `/control`, blocked-origin, clean quit, and uploaded artifact `7840118532`; `PACKAGING_VALIDATION.md` captures PASS results and `tools/packaging_validation_check.ps1` is expected to pass | None |
...
- Hosted macOS package build, `/control`, audio lifecycle, visible menu-bar/tray, tray `Open Control Panel`, blocked-origin, and clean-quit evidence now exists from package-smoke run `28074903385`; final release should keep `tools/packaging_validation_check.ps1` PASS.
- Current local Windows package build and package audio endpoint smoke evidence passes; P3-A2 remains complete.
...
- No roadmap gate is intentionally left open in the repo worksheets after the 2026-06-24 package-smoke evidence update; rerun final checks after any source/runtime/package change.
