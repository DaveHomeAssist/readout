...
| ID | Status | Current Evidence | Remaining Proof |
|---|---|---|---|
| P3-A1 | Partial hosted validation, pending macOS visual tray evidence refresh | `build_mac.sh` checks supported Python and `espeak-ng`; build-script tests cover preflight expectations; packaged entrypoint tests prove tray/control-panel routing; GitHub Actions package-smoke run `28073664040` built `dist/ReadOut.app`, passed `tools/mac_package_smoke.sh --app dist/ReadOut.app --include-audio`, verified `/control`, preview/stop/speak/stop audio lifecycle, blocked-origin, clean quit, and uploaded artifact `7839652216`; current branch adds `--include-tray-ui` to capture menu-bar screenshots, click tray `Open Control Panel`, and verify `READOUT_CONTROL_OPEN_PROBE`; `PACKAGING_VALIDATION.md` captures existing results; `tools/packaging_validation_check.ps1` still fails until visual tray/menu rows are passed or accepted as gaps | Run/refetch package-smoke with `--include-tray-ui`; if the runner cannot drive menu-bar UI, use human macOS target evidence or record accepted gaps |
...
- Hosted macOS package build, `/control`, audio lifecycle, blocked-origin, and clean-quit evidence now exists from package-smoke run `28073664040`. Current branch adds a tray UI smoke path to try to capture visible menu-bar/tray and tray menu selection evidence through `macos-tray-evidence`; final release should pass `tools/packaging_validation_check.ps1`.
