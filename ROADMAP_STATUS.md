...
- Manual smoke tests remain for Tk desktop audio, Chrome extension popup/runtime, `/control` audible preview/speak rows, packaged macOS lifecycle, and packaged Windows lifecycle. Use `tools/release_preflight.ps1 -RunSourceSmoke` first, `tools/server_smoke.ps1` for non-audio API/control checks, `tools/control_browser_runtime_smoke.ps1` for browser-rendered `/control` status evidence, `tools/control_browser_action_smoke.ps1` for rendered Save WAV/Stop evidence, `tools/tk_desktop_static_smoke.ps1` for Tk source-contract support evidence, `tools/tk_desktop_runtime_smoke.ps1` for non-audio Tk launch/config evidence, and `tools/manual_smoke_check.ps1` after filling interactive smoke evidence.
