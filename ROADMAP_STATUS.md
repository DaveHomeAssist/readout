...
- Current local Windows package build and package audio endpoint smoke evidence passes; final packaging validation remains blocked by macOS visible tray/menu-bar, tray Open Control Panel, and macOS audible lifecycle rows.
- Manual smoke tests remain for Tk desktop audio, Chrome extension Preview Voice audio, Chrome extension context-menu selected-text playback, `/control` audible preview/speak rows, and packaged macOS lifecycle. Use `tools/release_preflight.ps1 -RunSourceSmoke` first, `tools/server_smoke.ps1` for API/control checks with optional audio endpoint smoke, `tools/control_browser_runtime_smoke.ps1` for browser-rendered `/control` status evidence, `tools/control_browser_action_smoke.ps1` for rendered Save WAV/Stop evidence, `tools/extension_runtime_smoke.ps1` for extension popup READY/OFFLINE/origin/Stop command evidence, `tools/tk_desktop_static_smoke.ps1` for Tk source-contract support evidence, `tools/tk_desktop_runtime_smoke.ps1` for non-audio Tk launch/config evidence, and `tools/manual_smoke_check.ps1` after filling interactive smoke evidence.
